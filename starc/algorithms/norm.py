...
from starc.utils.simple_utils import timed, sample_space
...
def sample_reward_worker_dbase(reward: RewardCont,
                              state: np.ndarray,
                              action: np.ndarray,
                              next_state: np.ndarray):
  """Worker function that evaluates reward on pre-computed transitions"""
  return reward(state.tolist(), action.tolist(), next_state.tolist())
...
@timed
def norm_cont_with_dbase(reward: RewardCont,
                        states: np.ndarray,
                        actions: np.ndarray,
                        ...
                        n_samples: int = 10**3) -> float:
  """
  Compute norm using pre-computed transitions database instead of sampling.
  """
  if ord == 0: 
    return 1

  # Sample n_samples transitions from the database
  ...
  try:
    results = Parallel(n_jobs=-1, backend='threading')(
      delayed(sample_reward_worker_dbase)(reward, 
                                         states[i], 
                                         actions[i], 
                                         next_states[i])
      for i in indices)
