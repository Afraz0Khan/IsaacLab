...
class HumanoidEnvFixed:
    """
    Environment wrapper for Humanoid that provides STARC compatibility.
    
    This wrapper creates a gym-like interface compatible with STARC analysis
    for Humanoid environments, handling the observation space and StateVals training.
    ...
    # Humanoid observation space (matching Gymnasium Humanoid-v4: 376 dimensions)
    # This ensures compatibility between transition batch and STARC analysis
    state_space: Space = [(-100, 100)] * 376  # 376 dimensions to match Gymnasium Humanoid-v4
    
    # Humanoid action space (17 dimensions for joint torques)
    act_space: Space = [(-0.4, 0.4)] * 17
    ...
    def __init__(self, reward_func, discount: float, n_episodes_sarsa: int = 10000, **kwargs):
        ...
        # Create mock gym spaces for compatibility (match Gymnasium Humanoid-v4: 376 dimensions)
        self.observation_space = gym.spaces.Box(
            low=np.array([-100.0] * 376, dtype=np.float32),
            high=np.array([100.0] * 376, dtype=np.float32),
            dtype=np.float32,
        )
        
        self.action_space = gym.spaces.Box(
            low=np.array([-0.4] * 17, dtype=np.float32),
            high=np.array([0.4] * 17, dtype=np.float32),
            dtype=np.float32,
        )
        ...
        if n_episodes_sarsa and n_episodes_sarsa > 0:
            print(f"[INIT] HumanoidEnvFixed: n_episodes_sarsa={n_episodes_sarsa}")
        ...
        self.env_info = EnvInfoCont(
            trans_dist=HumanoidEnvFixed.predict_next_state,
            trans_dist_deterministic=True,
            discount=self.discount,
            state_space=HumanoidEnvFixed.state_space,
            action_space=HumanoidEnvFixed.act_space,
    ...
    def _generate_mock_observation(self) -> np.ndarray:
        """Generate a mock observation for STARC compatibility (376 dimensions to match Gymnasium Humanoid-v4)."""
        # In practice, this would be replaced by actual environment observation
        return np.random.normal(0, 1, size=(376,)).astype(np.float32)
    ...
    def _call_reward_function(self, action, obs, x_velocity):
        ...
        # Mock contact forces (zeros for Humanoid)
        contact_forces = np.zeros(2, dtype=np.float32)  # 2 feet
    ...
    @staticmethod
    def get_state_space():
        return HumanoidEnvFixed.state_space
    
    @staticmethod
    def get_action_space():
        return HumanoidEnvFixed.act_space
    
    @staticmethod
    def get_env_info():
        # Create a temporary instance to get env_info
        from starc.rewards.ground_truth_reward import GroundTruthReward
        temp_env = HumanoidEnvFixed(GroundTruthReward(), 0.99, 0)
    ...
    def data(self):
        ...
        class MockData:
            def __init__(self):
                # Mock contact forces (cfrc_ext) - 2 feet x 6 DOF each = 12 elements
                self.cfrc_ext = np.zeros(12, dtype=np.float32)
