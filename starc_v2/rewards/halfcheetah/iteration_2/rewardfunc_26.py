...
class RewardFunc_26(RewardFunc):
    """
    Discourages front-foot over-rotation.
    features: [x_vel, sin_front_foot, ctrl_cost]
    """
    def __init__(self):
        self._w = np.array([1.25, -0.20, -0.05])

    def __call__(self, env, state, action, next_state, x_velocity):
        if x_velocity is None: x_velocity=state[9]
        sin_front_foot = np.abs(np.sin(state[8]))
        ctrl_cost = np.sum(np.square(action))
        r = self._w[0]*x_velocity + self._w[1]*sin_front_foot + self._w[2]*ctrl_cost
