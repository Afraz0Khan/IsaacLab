...
class RewardFunc_25(RewardFunc):
    """
    Penalises vertical oscillations of torso.
    features: [x_vel, abs_torso_z_vel, ctrl_cost]
    """
    def __init__(self):
        self._w = np.array([1.2, -0.20, -0.05])

    def __call__(self, env, state, action, next_state, x_velocity):
        if x_velocity is None: x_velocity=state[9]
        abs_torso_z_vel = np.abs(state[10])
        ctrl_cost = np.sum(np.square(action))
        r = self._w[0]*x_velocity + self._w[1]*abs_torso_z_vel + self._w[2]*ctrl_cost
