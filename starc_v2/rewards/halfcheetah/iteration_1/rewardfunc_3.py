...
class RewardFunc_3(RewardFunc):
    """
    Penalises excessive joint velocity to produce smoother gaits.
    features: [x_velocity, ctrl_cost, joint_vel_sq]
    """
    def __init__(self):
        self._w = np.array([1.3, -0.05, -0.02])
    ...
    def __call__(self, env: HalfCheetahEnv, state, action, next_state, x_velocity):
        ...
        r = self._w[0]*x_velocity + self._w[1]*ctrl_cost + self._w[2]*joint_vel_sq
