...
class RewardFunc_39(RewardFunc):
    """
    Speed reward with smoothness via acceleration penalty.
    features: [x_vel, abs_accel, ctrl_cost]
    """
    def __init__(self):
        self._w = np.array([1.1, -0.4, -0.05])
    ...
    def __call__(self, env, state, action, next_state, x_velocity):
        ...
        abs_accel = np.abs(next_vx - state[9])
        ctrl_cost = np.sum(np.square(action))
        r = self._w[0]*x_velocity + self._w[1]*abs_accel + self._w[2]*ctrl_cost
    ...
    def get_features(self):
        return {
            "x_vel":"x_velocity",
            "abs_accel":"np.abs(next_state[9]-state[9])",
            "ctrl_cost":"np.sum(np.square(action))"
        }