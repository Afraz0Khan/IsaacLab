...
class RewardFunc_47(RewardFunc):
    """
    Encourages coordinated foot motion.
    features: [x_vel, sin_prod, ctrl_cost]
    """
    def __init__(self):
    ...
    def __call__(self, env, state, action, next_state, x_velocity):
        ...
        sin_prod = np.sin(state[8]) * np.sin(state[5])
        ctrl_cost = np.sum(np.square(action))
        r = self._w[0]*x_velocity + self._w[1]*sin_prod + self._w[2]*ctrl_cost
