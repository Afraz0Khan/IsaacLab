...
class RewardFunc_41(RewardFunc):
    """
    Encourages symmetric thigh angles.
    features: [x_vel, thigh_diff_sq, ctrl_cost]
    """
    def __init__(self):
        self._w = np.array([1.15, -0.3, -0.05])

    def __call__(self, env, state, action, next_state, x_velocity):
        if x_velocity is None: x_velocity=state[9]
        thigh_diff_sq = (state[3] - state[6])**2
        ctrl_cost = np.sum(np.square(action))
        r = self._w[0]*x_velocity + self._w[1]*thigh_diff_sq + self._w[2]*ctrl_cost
