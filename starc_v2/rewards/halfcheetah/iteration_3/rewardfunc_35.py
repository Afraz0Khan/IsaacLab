...
class RewardFunc_35(RewardFunc):
    """
    Synergy of speed and torso height.
    features: [speed_torso, ctrl_cost]
    """
    def __init__(self):
    ...
    def __call__(self, env, state, action, next_state, x_velocity):
        ...
        speed_torso = x_velocity * torso_z
        ctrl_cost = np.sum(np.square(action))
        r = self._w[0]*speed_torso + self._w[1]*ctrl_cost
    ...
    def get_features(self):
        return {
            "speed_torso": "x_velocity * env.get_body_com('torso')[2]",
            "ctrl_cost": "np.sum(np.square(action))"
        }